...
def auth(credentials: HTTPAuthorizationCredentials = Depends(securityBearer)):
    ...
    if existe is  None:
        ...
        if hora_bd != hora_hash:
            raise fastapi.HTTPException(status_code=430, detail="El token ya no es válido")
        else:
...
def validate_user(credentials: HTTPBasicCredentials = Depends(security)): 
    ...
    connx = sqlite3.connect("sql/usuarios.db") # conecta a la base de datos
    ...
    c.execute('UPDATE usuarios SET token = ?, timestamp = ? WHERE username = ? AND password = ?', (hashtoken, hora_actual_formateada, username, hashpassword))
    connx.commit() # ejecuta la actualizacion

    c.execute('SELECT token FROM usuarios WHERE username = ? AND password = ?', (username, hashpassword)) 
    existe = c.fetchone()
    if existe is None: 
        raise fastapi.HTTPException(status_code=401, detail="Not Authenticated")
    else:
